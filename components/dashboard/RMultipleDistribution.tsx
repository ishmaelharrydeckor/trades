// components/dashboard/RMultipleDistribution.tsx
"use client";
...
export default function RMultipleDistribution({ data }: Props) {
  const hasRData = data.some((b) => b.label !== "None" && b.trades > 0);

  return (
    <div className="rounded-2xl border border-[color:var(--border-panel)] bg-[color:var(--bg-panel)] p-5">
      <div className="mb-4">
        <h3 className="text-base font-semibold text-white">R-Multiple</h3>
        <p className="text-xs text-slate-500">
          {hasRData
            ? "Risk-adjusted distribution and performance"
            : "Awaiting trades with r_multiple data from your MT5 EA"}
        </p>
      </div>
